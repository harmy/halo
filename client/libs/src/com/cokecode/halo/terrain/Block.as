package com.cokecode.halo.terrain
{
	...
	public class Block extends Node2D implements IClip
	{
		...
		public function Block(url:String)
		{
			var tex:Texture2D = sTexDic[url];
			if (tex != null) {
				createSpriteSheet(tex);
				return;
			}
		...
		protected function createSpriteSheet(tex:Texture2D):void
		{
			tex.textureOptions = TextureOption.QUALITY_LOW;
			
			mSpriteSheet = new SpriteSheet(tex.bitmapWidth, tex.bitmapHeight, mSpriteWidth, mSpriteHeight, 1);
			
			// 创建精灵
			mSprite = new Sprite2D(tex);
			mSprite.setSpriteSheet(mSpriteSheet);
			mSprite.pivot.x = mSprite.width + 32;
			mSprite.pivot.y = mSprite.height * 3 + mSprite.height / 2;
			
			mSpriteSheet.frame = mFrame;
